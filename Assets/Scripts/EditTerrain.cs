...
public static class EditTerrain
{
	...
	public static bool SetBlock(Vector3 pos, Block block){
		World world = GameObject.Find("World").GetComponent<World>();
		WorldPos posi = GetBlockPos(pos);
		Chunk chunk = world.GetChunk(posi.x, posi.y, posi.z);
		chunk.world.SetBlock(posi.x, posi.y, posi.z, block);
